...
RUN if [[ "x$BINARY_FILE" != "xencointer-client" ]] ; then ./integritee init-shard; fi
RUN if [[ "x$BINARY_FILE" != "xencointer-client" ]] ; then ./integritee shielding-key; fi
RUN if [[ "x$BINARY_FILE" != "xencointer-client" ]] ; then ./integritee signing-key; fi
RUN if [[ "x$BINARY_FILE" != "xencointer-client" ]] ; then ./integritee mrenclave > ~/mrenclave.b58; fi
